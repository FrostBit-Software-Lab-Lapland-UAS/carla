...
void ARadar::SendLineTraces(float DeltaTime)
{
	...
	// Example: for 12.0 precipitation value (50 mm/hr rain) drop off is ~26 % like in the paper above
	float dropOffRate = (precipitation * 2.08) / 100;
	if (dropOffRate > 0.55) {
		dropOffRate = 0.55;										// cap drop off rate to 55% (maximum drop off rate they saw in the paper)
	}
	dropOffRate = (100 - (precipitation * dropOffRate)) / 100;	// drop off rate, range: 0.45 to 1.00

	// Calculate new Radar Range
	// Obviously there's no clear Radar drop off rate in foggy conditions (it's complicated, see paper above) 
