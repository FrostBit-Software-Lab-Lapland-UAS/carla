...
bool ACustomRayCastSemanticLidar::CalculateNewHitPoint(FHitResult& HitInfo, float rain_amount, float particle_size, FVector end_trace, FVector LidarBodyLoc) const
{
  ...
  float r = (float)rand() / double(RAND_MAX); //random between 0-1
    ...
    HitInfo.Component = nullptr; //to identify the snowflake later, set component to null if hitpoint is snowflake
		return true;
	}
	else {
		return false;
