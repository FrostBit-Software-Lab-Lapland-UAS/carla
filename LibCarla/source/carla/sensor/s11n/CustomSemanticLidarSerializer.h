...
namespace s11n {
  ...
  protected:
    friend class CustomSemanticLidarSerializer;

    explicit CustomSemanticLidarHeaderView(const uint32_t *begin) : _begin(begin) {
  ...
  public:
    ...
    static CustomSemanticLidarHeaderView DeserializeHeader(const RawData &data) {
      return CustomSemanticLidarHeaderView{reinterpret_cast<const uint32_t *>(data.begin())};
    }

    static size_t GetHeaderOffset(const RawData &data) {
      auto View = DeserializeHeader(data);
      return sizeof(uint32_t) * (View.GetChannelCount() + data::CustomSemanticLidarData::Index::SIZE);
    }

    template <typename Sensor>
    static Buffer Serialize(
        const Sensor &sensor,
        const data::CustomSemanticLidarData &measurement,
        Buffer &&output);
  ...
  template <typename Sensor>
  inline Buffer CustomSemanticLidarSerializer::Serialize(
      const Sensor &,
      const data::CustomSemanticLidarData &measurement,
      Buffer &&output) {
