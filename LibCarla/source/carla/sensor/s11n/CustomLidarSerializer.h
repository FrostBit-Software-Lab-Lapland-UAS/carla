...
namespace s11n {
  ...
  class CustomLidarHeaderView {
    using Index = data::CustomLidarData::Index;
  ...
  private:
    friend class CustomLidarSerializer;

    explicit CustomLidarHeaderView(const uint32_t *begin) : _begin(begin) {
  ...
  public:
    ...
    static CustomLidarHeaderView DeserializeHeader(const RawData &data) {
      return CustomLidarHeaderView{reinterpret_cast<const uint32_t *>(data.begin())};
    }

    static size_t GetHeaderOffset(const RawData &data) {
      auto View = DeserializeHeader(data);
      return sizeof(uint32_t) * (View.GetChannelCount() + data::CustomLidarData::Index::SIZE);
    }

    template <typename Sensor>
    static Buffer Serialize(
        const Sensor &sensor,
        const data::CustomLidarData &data,
        Buffer &&output);
  ...
  template <typename Sensor>
  inline Buffer CustomLidarSerializer::Serialize(
      const Sensor &,
      const data::CustomLidarData &data,
      Buffer &&output) {
