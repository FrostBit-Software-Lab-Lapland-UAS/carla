...
namespace data {
  ...
  class CustomSemanticLidarMeasurement : public Array<data::CustomSemanticLidarDetection>  {
    static_assert(sizeof(data::CustomSemanticLidarDetection) == 6u * sizeof(float), "CustomSemanticLidarDetection size missmatch");
    using Super = Array<data::CustomSemanticLidarDetection>;

  protected:
    using Serializer = s11n::CustomSemanticLidarSerializer;

    friend Serializer;

    explicit CustomSemanticLidarMeasurement(RawData &&data)
