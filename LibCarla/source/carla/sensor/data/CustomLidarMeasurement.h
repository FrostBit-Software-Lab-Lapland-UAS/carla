...
namespace data {
  ...
  class CustomLidarMeasurement : public Array<data::CustomLidarDetection>  {
    static_assert(sizeof(data::CustomLidarDetection) == 4u * sizeof(float), "Location size missmatch");
    using Super = Array<data::CustomLidarDetection>;

  protected:
    using Serializer = s11n::CustomLidarSerializer;

    friend Serializer;

    explicit CustomLidarMeasurement(RawData &&data)
