...
namespace s11n {
  class CustomSemanticLidarSerializer;
  class CustomSemanticLidarHeaderView;
}
...
namespace data {
  ...
  #pragma pack(push, 1)
  class CustomSemanticLidarDetection {
    public:
      geom::Location point{};
      float cos_inc_angle{};
      uint32_t object_idx{};
      ...
      CustomSemanticLidarDetection() = default;

      CustomSemanticLidarDetection(float x, float y, float z, float cosTh, uint32_t idx, uint32_t tag) :
          point(x, y, z), cos_inc_angle{cosTh}, object_idx{idx}, object_tag{tag} { }
      CustomSemanticLidarDetection(geom::Location p, float cosTh, uint32_t idx, uint32_t tag) :
  ...
  public:
    explicit CustomSemanticLidarData(uint32_t ChannelCount = 0u)
      : _header(Index::SIZE + ChannelCount, 0u) {
      _header[Index::ChannelCount] = ChannelCount;
    }

    CustomSemanticLidarData &operator=(CustomSemanticLidarData &&) = default;

    virtual ~CustomSemanticLidarData() {}
  ...
  private:
    std::vector<CustomSemanticLidarDetection> _ser_points;

  friend class s11n::CustomSemanticLidarHeaderView;
  friend class s11n::CustomSemanticLidarSerializer;
  };
