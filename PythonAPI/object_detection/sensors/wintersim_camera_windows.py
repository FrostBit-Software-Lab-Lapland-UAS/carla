...
class CameraWindows(threading.Thread):
    ...
    def render_front_rgb_camera(self):
        ...
        if self.front_rgb_image is not None:
            ...
            image = self.object_detection_deep_sort.Input(image)
            end = time.perf_counter()
            mm = (end - start) * 1000
            print(f"detection took {mm:0.4f} milliseconds")
    ...
    def __init__(self, ego_vehicle, camera, world, actor_name):
        ...
        self.object_detection_deep_sort = WinterSimObjectDetectionDeepSort()
